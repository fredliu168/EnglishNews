...
def queryWord(word):
    REDIS_WORD_TIME = current_app.config['REDIS_WORD_TIME']  # redis 保持时间

    # 有道查询接口
    YOUDAO_API = current_app.config['YOUDAO_API']
    YOUDAO_APPID = current_app.config['YOUDAO_APPID']
    YOUDAO_KEY = current_app.config['YOUDAO_KEY']
    ...
    sign = MD5(YOUDAO_APPID + word + salt + YOUDAO_KEY)

    url = '{YOUDAO_API}?q={word}&from=en&to=zh_CHS&appKey={appKey}&salt={salt}&sign={sign}'.format(
        YOUDAO_API=YOUDAO_API, word=word, appKey=YOUDAO_APPID, salt=salt, sign=sign)
    ...
    if rs is not None and reponse.status_code == 200:
        ...
        # print(json_str)
        rs.set(word, reponse.text, ex=REDIS_WORD_TIME)  # 设置过期时间
